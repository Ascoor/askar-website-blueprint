...
export default function HeroSlider() {
  ...
  const textParentVariants = {
    initial: { opacity: 0, y: 48, filter: "blur(15px)", scale: 0.98 },
    animate: { opacity: 1, y: 0, filter: "blur(0px)", scale: 1 },
    exit: { opacity: 0, y: -32, scale: 1.04, filter: "blur(8px)" }
  };
  const textChildVariants = {
    initial: { opacity: 0, x: isRTL ? 96 : -96, scale: 0.94, filter: "blur(14px)" },
    animate: { opacity: 1, x: 0, scale: 1, filter: "blur(0px)" },
    exit: { opacity: 0, x: isRTL ? -28 : 28, scale: 1.07, filter: "blur(8px)" }
  };
...
<AnimatePresence mode="wait">
  ...
  />
      ...
      {/* Text Block - absolutely positioned half-page, vertically centered */}
      <div
        className={`
          absolute top-0 ${isRTL ? "right-0" : "left-0"}
